...
class ReferenceClientsForm(QWidget):
    def __init__(self, parent=None):
        ...
        # 8 столбцов: id, name, country_id, bank_id, inn, address, zip_code, email
        self.table.setColumnCount(8)
        self.table.setHorizontalHeaderLabels([
            "ID", "Название", "СтранаID", "БанкID", "ИНН", "Адрес", "Индекс", "Email"
        ])
    ...
    def add_record(self):
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO Clients (name, country_id, bank_id, inn, address, zip_code, email)
            VALUES (?, ?, ?, ?, ?, ?, ?)
