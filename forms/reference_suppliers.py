...
class ReferenceSuppliersForm(QWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Справочник: Поставщики (Suppliers)")
    ...
    def load_data(self):
        ...
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, name FROM Suppliers ORDER BY id ASC")
    ...
    def edit_record(self):
        ...
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Suppliers SET name = ? WHERE id = ?", (name, record_id))
        conn.commit()
        conn.close()
        self.load_data()
    ...
    def print_data(self):
        QMessageBox.information(self, "Печать", "Здесь могла бы быть ваша печать списка поставщиков...")
