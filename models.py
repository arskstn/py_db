...
class MenuItem:
    ...
    def create_initial_menu_items():
        """
        Заполняет таблицу MenuItems начальными данными,
        если она пуста.
        """
        conn = get_connection()
        cursor = conn.cursor()

        # Проверим, есть ли уже записи в MenuItems
        ...
        # Описываем структуру меню (ID, parent_id, name, dll_name, func_name, display_order)
        # ID указываем вручную для удобства, но можно и автогенерацию
        menu_items = [
            # Уровень 1
...
class User:
    ...
    def create_superuser_if_not_exists():
        """
        Создаёт суперпользователя admin/admin, если его нет.
        """
        conn = get_connection()
        cursor = conn.cursor()
